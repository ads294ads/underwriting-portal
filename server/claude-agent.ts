  ...
  try {
    ...
    try {
      ...
      if (response.content && response.content.length > 0) {
        ...
        if ('type' in firstContent && firstContent.type === 'text' && 'text' in firstContent) {
          contentText = String(firstContent.text);
        } else {
...
function combineOwnerAnalyses(ownerAnalyses: any[]): any {
  ...
  // Convert Sets to Arrays where needed
  return {
    overview: combinedOverview,
    legalIssues: allLegalIssues,
    financialRedFlags: allFinancialRedFlags,
    reputationInsights: allReputationInsights,
    managementCapabilities: allManagementCapabilities,
    ...
    highRiskFactors: allHighRiskFactors,
    moderateRiskFactors: allModerateRiskFactors,
    mitigatingFactors: allMitigatingFactors
  };
}
